...
function draw() {
  ...
  vid1.loadPixels();
   var stepSize = round(constrain(mouseX/8, 2, 10));
  ...
  for (var y=0; y<height; y+=stepSize) {
    for (var x=0; x<width; x+=stepSize) {
      ...
      fill(vid1.pixels[index]*random(1,2),vid1.pixels[index+1]*random(1,2),vid1.pixels[index+2]*random(1,2),random(20));
      //rect(x*14+random(2*y), y*random(10,14)+random(2*x), radius*random(2,4), radius*random(2,8));
      ellipse(x*2, y*2, random(5,200), random(5,200));
    }
  }
